...
vRP.Prepare('skinweapon/Create', " INSERT IGNORE INTO havan_skins(component, stock) VALUES (@component, @stock) ")
vRP.Prepare('skinweapon/getStock', "SELECT * FROM havan_skins")
vRP.Prepare('skinweapon/getStockSpecific', "SELECT * FROM havan_skins WHERE component = @component")
vRP.Prepare("skinweapon/setSkin","INSERT INTO havan_skins_users(user_id, skins) VALUES(@user_id, @skins)")
vRP.Prepare('skinweapon/updateSkin', "UPDATE havan_skins_users SET skins = @skins WHERE user_id = @user_id")
vRP.Prepare('skinweapon/updateSkinEquip', "UPDATE havan_skins_users SET equipadas = @equipadas WHERE user_id = @user_id")
vRP.Prepare('skinweapon/getPlayerSkins', "SELECT * FROM havan_skins_users WHERE user_id = @user_id")

vRP.Prepare('skinweapon/updateSkinStock', "UPDATE havan_skins SET stock = @stock WHERE component = @component")
...
AddEventHandler("skinweapon:check", function(callback)
    local source = source
    local user_id = vRP.Passport(source)
    ...
    if source and user_id then
        ...
        for _, weapon in pairs(weapons) do
            ...
            local weaponComponent = EquipWeapons[user_id] and EquipWeapons[user_id][weaponName] or nil
            -- Enviar o componente da skin ao cliente
            if callback then
                callback(weaponComponent)
            end
        end
    end
end)]]
...
RegisterTunnel.RequestUnequip = function(data)
  ...
  if user_id then
      ...
      -- Variável para controlar se o jogador possui a arma
      local hasWeapon = false

      -- Itera sobre as armas para verificar se o jogador tem a arma equipada
      for _, weapon in ipairs(weapons) do
          if weapon.name == skinglobal[data][3] then
              hasWeapon = true
              break
      ...
      -- Verifica se o jogador possui a arma equipada
      if hasWeapon then
          -- Verifica se o jogador tem o componente equipado
          if EquipWeapons[user_id] and EquipWeapons[user_id][skinglobal[data][3]] then
              -- Remove o componente da arma
              RemoveWeaponComponentFromPed(GetPlayerPed(source), GetHashKey(skinglobal[data][3]), data)

              -- Remove o componente da lista de componentes equipados
              EquipWeapons[user_id][skinglobal[data][3]] = nil

              TriggerClientEvent("Notify", source, "sucesso", "Skin removida com sucesso.")
          else
              TriggerClientEvent("Notify", source, "negado", "Você não possui esse componente equipado para remover.")
          end
      else
          TriggerClientEvent("Notify", source, "negado", "Você não possui essa arma equipada para remover o componente.")
      end
  end
end
...
--[[RegisterTunnel.RequestUnequip = function(data)
  ...
  if user_id then
    local weapons = vRPclient.getWeapons(source)
    if weapons[skinglobal[data][3]] --then -- nao encontra e passa para o else
      --[[EquipWeapons[user_id][skinglobal[data][3]] --= nil
      --[[RemoveWeaponComponentFromPed(GetPlayerPed(source),skinglobal[data][3],data)
      TriggerClientEvent("Notify",source,"sucesso","Skin removida com sucesso.")
    else
      TriggerClientEvent("Notify",source,"negado","Você 2não possui essa arma equipada para remover esse componente.")
    end
  end
end]]
