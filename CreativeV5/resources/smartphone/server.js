...
if (pB.base === 'creative_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
    ...
    if (pB.base === 'creative_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        ...
        } else pB.base === 'NewWave_network' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passport, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.hasPermission = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
...
(pB.base === 'creative_v4' || pB.base === 'new_NewWave') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    const d = [];
...
const pF = {
    ...
    'getUData'(d, e) {
        ...
        f.user_id = d;
        f.dkey = e;
        console.log(f.user_id,f.dkey,'esse é o select amigao');
...
const pN = {
    'new_NewWave': 'checkBroken',
    'NewWave_network': 'CheckDamaged'
};
const pO = pN;
    ...
    try {
        while (true) {
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[SMARTPHONE] AUTENTICADO RLN DEV'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
...
pF.ready(async () => {
    ...
    f = {
        ...
        // 'characters': 'fines', descomenta caso for usar a multa no character, se manter comentado ele vai pegar primeiro o 'fines': 'Value' 
        // lembrar sempre usar o commit da versão 3.0 para corrigir algum erro no banco do celular!
        'fines': 'Value'
    };
    const g = f;
    ...
    for (let [k, l, m] of [['fines', 'text', 'price'], ['fines', 'text', 'price'], ['wise_multas', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['vrp_multas', 'motivo', 'valor']]) {
        e.push({
            'table': k,
    ...
    i = pF.firstTable('vrp_banco', 'vrp_bdl_banco', 'smartphone_bank', 'smartphone_bank_invoices');
