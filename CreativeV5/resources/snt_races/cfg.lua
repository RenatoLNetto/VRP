...
get_user_source_by_user_id = function(user_id)
    return vRP.GetPlayerPed(user_id)--vRP.getUserSource(user_id)
end    
--
...
get_user_id_by_source = function(source)
    return vRP.Passport(source)--GetPlayers()--vRP.getUserId(source)
end
...
get_char_name_by_user_id = function(Passaport)
    local identity = vRP.Identity(Passaport)
    --print(vRP.Identity(Name)(Passaport), '123')
    print(identity, 'identity')
    if identity.name2 then
        print(identity.name, 'NOME')
        print(identity.name2, 'SOBRENOME')
        print('yess')
...
remove_race_ticket = function(user_id)
    if vRP.ConsultItem(user_id,    "dollars",    1,     true) then
        print(vRP.ConsultItem)
        return true
    end
    TriggerClientEvent("Notify",    vRP.Passport(user_id),    "negado",    "Você precisa de um Ticket de Corrida!",    10000)
    return false
end
...
pay_race_prize = function(user_id,    prize)
   --[[ local cops = vRP.getUsersByPermission('policia.perm')
    ...
    print('entrega dinheiro')
    vRP.GiveItem(user_id,"dollars2",prize)
    print('ENTREGOU')
end
...
explosive_races = {
    {
        ...
        prize = 80000,    
        points = {
            {2279.36,3149.08,48.48},
            {2307.92,3115.46,47.28}
        }
    },
    {
    ...
    {
        name = "Menino de FIVEM!",    
        needed = nil,    
        desc = "Mostre que Vc Domina FIVEM!",    
        time = 255,    
        prize = 110000,    
        points = {
            {283.33,     746.03,     183.02},    
