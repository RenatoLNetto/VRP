Citizen.CreateThread(function()
    ...
    prepare("check_race_data", "SELECT * FROM snt_races_data WHERE user_id = ? AND race_name = ? AND data_type = ?")
    prepare("insert_race_data", "INSERT INTO snt_races_data (user_id, race_name, data_type, data_value) VALUES (?, ?, ?, ?)")
    prepare("update_race_data", "UPDATE snt_races_data SET data_value = ? WHERE user_id = ? AND race_name = ? AND data_type = ?")
    prepare("snt_races/insertOrUpdateData", [[
        INSERT INTO snt_races_data(user_id, race_name, data_type, data_value)
        VALUES (@user_id, @race_name, @data_type, @data_value)
...
function vRP.InsertRaceData(u, rn, d, dv)
    local exists = vRP.Query("check_race_data", {user_id = u, race_name = rn, data_type = d})
    
    if #exists > 0 then
       -- O registro já existe, pode fazer um update ou decidir o que fazer
       local success, err = vRP.Execute("update_race_data", {dv, u, rn, d})
       if not success then
          print("Erro ao atualizar dados:", err)
       end
    else
       -- O registro não existe, pode inserir
       local success, err = vRP.Execute("insert_race_data", {u, rn, d, dv})
       if not success then
 ...
 function _store_stats(u, rn, d, dv)
    if d == 0 or d == 1 then
        if d == 1 then
            ...
            -- Usando INSERT com ON DUPLICATE KEY UPDATE
            execute("snt_races/insertOrUpdateData", { user_id = u, race_name = rn, data_type = d, data_value = dv })
            print('gg')
...
AddEventHandler("snt_races/srequest/start_race",function(rn)
    ...
    if r then
        if explosive_races[r].needed ~= nil then
            if vRP.ConsultItem(user_id,"dollars",1) then
                vRP.RemoveItem(user_id,"dollars",1)
                ...
                TriggerClientEvent("Notify",s,"aviso","Voce não possui ticket de corrida1",5000)
            end
            return
        end
        vars.racing_players[u] = r
