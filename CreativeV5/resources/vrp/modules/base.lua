...
function vRP.Execute(Name, Params)
    local success, result = pcall(function()
        return exports.oxmysql:execute(Prepares[Name], Params)
    end)

    if not success then
        print("Erro ao executar a query:", result)
        return false, result
    end
...
local vehicles = {}
function vRP.vehicleGlobal2(vehicles)
    return vehicleGlobal2(vehicles)
end

local List = {}
function vRP.itemlist2(List)
    return itemlist2(List)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- IDENTITIES
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vRP.ApplyTemporary(Passport,source)
    ...
    if Prepares[Passport] and Datatable then
        ...
        TriggerClientEvent("hud:Armour",source,Datatable["Armour"])
                        ...
                        ["body"] = {
                                        ...
                                        ["columns"] = {
                                            {
                                                ...
                                                ["items"] = {
                                                    {
                                                        ["type"] = "TextBlock",
                                                        ["text"] = "Olá, "..name.."! Prepare-se para mergulhar na vida vibrante e imprevisível de SANTOS RP! Aqui, cada escolha molda o seu destino e cada ação tem uma consequência. Junte-se a outros jogadores, estabeleça alianças, enfrente adversários e construa sua própria história neste vasto universo roleplay. Lembre-se: Respeite as regras, entre no personagem e divirta-se!",
                                                        ["isSubtle"] = true,
                                                        ["wrap"] = true,
                                                    },
                            ...
                            },
                            {
                                ...
                                ["items"] = {
                                    ...
                                    {
                                        ["type"] = "Image",
                                        ["url"] = "https://cdn.discordapp.com/attachments/1291601539198095391/1296883213876068452/logo.png?ex=6713e81d&is=6712969d&hm=d7f1c5fbace33c033146c86d69f672e39bc871f32cb442240dd08c081d6a0c21&",
                                    }, 
                                    {
                                        ["type"] = "ColumnSet",
                                        ["columns"] = {
                                            {
                                                ...
                                                ["items"] = {
                                                    {
                                                        ["type"] = "TextBlock",
                                                        ["text"] = "1 PASSO: COPIE SEU TOKEN ABAIXO E COLE NA SALA  DO DISCORD",
                                                        ["size"] = 'Small', 
                                                        ["wrap"] = true
                                        ...
                                        ["actions"] = {
                                            {
                                                ...
                                                ["id"] = 'copy_to_token',
                                                ["title"] = 'SEU TOKEN ',
                                                ["iconUrl"] = 'https://cdn.discordapp.com/attachments/1291601539198095391/1296883213876068452/logo.png?ex=6713e81d&is=6712969d&hm=d7f1c5fbace33c033146c86d69f672e39bc871f32cb442240dd08c081d6a0c21&'
                                            },
                                        },
                                    },
                                    {
                                        ...
                                        ["actions"] = {
                                            {
                                                ["type"] = "Action.OpenUrl",
                                                ["id"] = 'copy_to_discord',
                                                ["title"] = 'https://discord.gg/zVE8QHXU5M',
                                                ["url"] = 'https://discord.gg/zVE8QHXU5M',
                                                ["iconUrl"] = 'https://cdn.discordapp.com/attachments/1291601539198095391/1296883213876068452/logo.png?ex=6713e81d&is=6712969d&hm=d7f1c5fbace33c033146c86d69f672e39bc871f32cb442240dd08c081d6a0c21&'
                                            },
                                        },
                                    },
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- SISTEMA DE AUTENTICAÇÃO NEW WAVE STORE
-----------------------------------------------------------------------------------------------------------------------------------------
local ip = "89.213.41.174" -- "seu_ip_permitido_aqui"
local id = ".renatonetto." -- "seu_discord_id"
local nome = "SANTOS RP" --"nome_do_cliente"


PerformHttpRequest('http://api.ipify.org/', function(errorCode, resultData, resultHeaders)

    if ip == tostring(resultData) then
        print('^2IP AUTENTICADO, VERSÃO 5.0 RLN Network')
       -- print("^2 SCRIPT AUTHENTICADO COM SUCESSO !!!")   
        print("^2 IP: ^5"..resultData)   
        print("^2 by BASE RLN Adquira já !!!") 
        print("^1 https://discord.gg/.renatonetto.") 
         -- Log para o Discord
         local message = string.format("**Base RLN Network V 2.0**\n**IP AUTENTICADO:** %s\n**ID DISCORD:** %s\n**NOME CLIENTE:** %s\n**Horário:** %s ", ip, id, nome, os.date("%H:%M:%S"))
         TriggerEvent("SistemaRLN","Autenticacao", message) -- Substitua "admin" pelo canal de registro apropriado

    else
        print('^1IP NÃO AUTENTICADO')
        local message = string.format("Base RLN Network V 2.0\n**IP NÃO AUTENTICADO:** %s\n**ID DISCORD:** %s\n**NOME CLIENTE:** %s\n**Horário:** %s ", ip, id, nome, os.date("%H:%M:%S"))
        TriggerEvent("SistemaRLN","Autenticacao", message) -- Substitua "admin" pelo canal de registro apropriado
...
SistemasDiscord = {	
	["Autenticacao"] = "https://discord.com/api/webhooks/1040263945384116234/1sNSb273Ut39s5WmN_A_2oG41o1XLfGkh00Iw2YYpzmAjlJKK7Y7RgRnZdwnPSXsecq3",--"https://discordapp.com/api/webhooks/960666050683351121/uw5FCawGD5K1f3esk8Cr9u97gaAcMtUcpKOyfGL5STLTzBVZIxAnnWJxnMGRyhAyhMtc", -- não remover e não trocar
}

-----------------------------------------------------------------------------------------------------------------------------------------
-- -- SISTEMA DE AUTENTICAÇÃO NEW WAVE STORE
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("SistemaRLN",function(Hook,Message,Color)
	PerformHttpRequest(SistemasDiscord[Hook],function(err,text,headers) end,"POST",json.encode({
		username = ServerName,
