...
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKDAMAGED NEW VERSION
-----------------------------------------------------------------------------------------------------------------------------------------
--[[function vRP.CheckDamaged(Item) 
    local itemSplit = splitString(Item, "-") -- Armazenando o resultado da função splitString

    -- Verificar se itemDurability(Item) é válido e se itemSplit[2] existe
    if itemDurability(Item) and itemSplit[2] then
        local itemTime = parseInt(itemSplit[2]) -- Converter o itemSplit[2] para inteiro
        local currentTime = os.time() -- Tempo atual

        -- Calcular a porcentagem da durabilidade restante
        local durabilityPercentage = parseInt((3600 * itemDurability(Item) - parseInt(currentTime - itemTime)) / (3600 * itemDurability(Item)) * 100)

        -- Verificar se a durabilidade é menor ou igual a 1
        if durabilityPercentage <= 1 then
            return true
        end
    end

    -- Se não for danificado, retornar false
    return false
end]]
function vRP.CheckDamaged(Item)
